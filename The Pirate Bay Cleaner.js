...
*/
  ...
  $('#searchResult > tbody > tr').each(function (i) {
    ...
    if (!torrentHTML.match(/alt="Next"|alt="Previous"/gi)) {
      ...
      if (sizecontrol == true) {
        if (single == true) var torrentSizeHTML = torrentHTML.match(/(\d+.\d+|\d+)(&nbsp;)(GiB|MiB|KiB)/i)[0]; else var torrentSizeHTML = torrentHTML.match(/(Size )(\d+.\d+|\d+)(&nbsp;)(GiB|MiB|KiB)/i)[0];
        var torrentSize = torrentSizeHTML.match(/\d+.\d+|\d+/)[0];
        var torrentSizeType = torrentSizeHTML.match(/GiB|MiB|KiB/i)[0];
        if (torrentSizeType == 'GiB') torrentSize *= (1024 * 1024); else if (torrentSizeType == 'MiB') torrentSize *= 1024;
        var calcedAllowedSize = sizecontrol_size;
        if (sizecontrol_type == 'GiB') calcedAllowedSize *= (1024 * 1024); else if (sizecontrol_type == 'MiB') calcedAllowedSize *= 1024;
